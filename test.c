...
void plusone_upper(unsigned int i, char *c)
{
	if (i)
		*c += 1;
	else
...
char plusone_ret(char c)
{
	c += 1;
	return (c);
}

char plusone_upper_ret(unsigned int i, char c)
{
	if (i)
		c += 1;
	else
...
void	ft_initmem(void *s1, void *s2, void *s3)
{
	bzero(s1, SIZE);
	bzero(s2, SIZE);
	bzero(s3, SIZE);
...
char				**ft_ref_strsplit(char const *s, char c)
{
	char	**tab;
...
//	ISALPHA
	if (!ft_isalpha('A') || !ft_isalpha('a') || ft_isalpha(']'))
		printf("[isalpha]                                                     -----KO-----\n");
	else
		printf("[isalpha]                                                       |||OK|||\n");
...
//	ISALNUM
	if (!ft_isalnum('1') || !ft_isalnum('a') || ft_isalnum(']'))
		printf("[isalnum]                                                     -----KO-----\n");
	else
		printf("[isalnum]                                                       |||OK|||\n");
...
//	ISPRINT
	if (ft_isprint(037) || ft_isprint(0177) || ft_isprint(0200)
		 || !ft_isprint(040) || !ft_isprint(073) || !ft_isprint(0153))
		printf("[isprint]                                                     -----KO-----\n");
	else
		printf("[isascii]                                                       |||OK|||\n");
...
//	TOLOWER
	...
	s2[0] = 'A';
	s2[0] = ft_tolower('A');
	ft_memdiff(s1, s2, s3, "valid argument", SIZE);
	s1[0] = 0x24;
	s2[0] = ft_tolower(0x24);
	ft_memdiff(s1, s2, s3, "invalid argument", SIZE);
